...
var selectTextDisplay = 'Selecione [NUMBER] os números'; //select number text display
...
var prizeTableDisplay = 'Tabela de Premiação'; //score table text display
var numberTextDisplay = 'Seus números'; //your score text display
var matchTextDisplay = '[NUMBER] ACERTOS'; //match text display
var scoreTextDisplay = 'R$[NUMBER]'; //prize score text display

var bonusBall = false; //toggle bonus ball
var bonusTextDisplay = 'ACERTOU [NUMBER] + BONUS'; //match bonus text display
...
// var score_arr = [
// 	{ prize: 5000, percent: 10 },
// 	{ prize: 100, percent: 3 },
// 	{ prize: 500, percent: 0 },
// 	{ prize: 1000, percent: 0 },
// 	{ prize: 10000, percent: 0 },
// 	{ prize: 100000, percent: 0 }
// ];
...
var exitMessage = 'VOCÊ TEM CERTEZA\nQUE QUER SAIR DO JOGO?'; //go to main page message
var resultCompleteText = 'VOCÊ GANHOU R$[NUMBER]!'; //result complete text display
var resultFailText = 'VOCÊ NÃO GANHOU!';  //result fail text display
...
function goPage(page) {
	...
	switch (page) {
		...
		case 'result':
			...
			$("#saldo_atual").html(saldo_atual.toLocaleString('pt-br', {minimumFractionDigits: 2}));
...
function registerStart(n) {
	return $.ajax({
		type: "POST",
/*		url: '/db/startLoteria.php', */
		url: 'https://megafacil.fun/db/startLoteria.php',
		data: {
			numeros: n
		}
	})
...
function startGame() {
	...
	for (var n = 0; n < totalNum; n++) {
		...
		//cor acertos
		$.prize['text' + n].color = $.prize['score' + n].color = "#000";
	}
...
async function startSpin() {
	...
	await registerStart(gameData.selectArray).done(function (result) {
		registraRodada = result;
		console.log(registraRodada);
	}).fail(function(jqXHR, textStatus, errorThrown) {
        console.error("Ocorreu um erro:", textStatus, errorThrown);
    });


	if (!registraRodada.run) {
	...
	gameData.revealArray = registraRodada.reveal.map(Number);
	...
	if (gameData.selectArray.length == score_arr.length) {
		...
		var saldo_atual_h = $("#saldo_atual").html();
		saldo_atual_h = saldo_atual_h.replace(',','.');
		saldo_atual_h = parseFloat(saldo_atual)-parseFloat(valor_entrada);
		$("#saldo_atual").html(saldo_atual_h.toLocaleString('pt-br', {minimumFractionDigits: 2}));
		saldo_atual = registraRodada.saldo_atual;
...
function revealWinNumber() {
	...
	TweenMax.to(targetBall, .5, {
		x: gameData.sphereX, y: canvasH / 100 * 71, rotation: 0, scaleX: 1, scaleY: 1, overwrite: true, onComplete: function () {
